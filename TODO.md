...
- `System::trackPower(bool on)`, `System::stop()` as well as `Programming::cvRead(uint16_t cv_addr)` and `Programming::cvWrite(uint16_t cv_addr, uint8_t byte)` should probably return a bool to indicate success? This would help in accidentially sending any broadcast replies although some command might have failed.
